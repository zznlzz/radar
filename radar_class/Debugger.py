...
class Debugger(object):
    ...
    def pred_loc_debugger(self, pred_loc):
        ...
        if (datetime.now() - self.start_time).total_seconds() >= self.loc_output_interval:
            self.panel.update_text(f'The x: {self.distance_sum_x/self.calculator}, y: {self.distance_sum_y/self.calculator}, z: {self.distance_sum_z/self.calculator}')
            # 使用完毕后要及时归零
            self.calculator = 0
        else:
            pass