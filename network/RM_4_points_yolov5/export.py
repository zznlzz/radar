...
if __name__ == '__main__':
    ...
    # Input
    img = torch.zeros(opt.batch_size, 3, *opt.img_size, device='cuda:0')  # image size(1,3,320,192) iDetection
    ...
    if opt.onnx_infer:
        ...
        pred = non_max_suppression_face(torch.tensor(y_onnx, device='cuda:0'))
    ...
    if opt.onnx2trt:
        ...
        print('\nStarting TensorRT...')
        torch2trt.trt_model.ONNX_to_TRT(onnx_model_path=f,trt_engine_path=f.replace('.onnx', '.trt'),fp16_mode=opt.fp16_trt)
